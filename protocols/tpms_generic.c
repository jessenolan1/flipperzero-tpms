...
SubGhzProtocolStatus tpms_block_generic_serialize(TPMSBlockGeneric *instance,
  ...
  do {
    ...
    if (!flipper_format_write_header_cstr(flipper_format, TPMS_KEY_FILE_TYPE,
                                          TPMS_KEY_FILE_VERSION)) {
      FURI_LOG_E(TAG, "Unable to add header");
      res = SubGhzProtocolStatusErrorParserHeader;
      break;
    }

    if (!flipper_format_write_uint32(flipper_format, "Frequency",
                                     &preset->frequency, 1)) {
    ...
    tpms_block_generic_get_preset_name(furi_string_get_cstr(preset->name),
                                       temp_str);
    ...
    if (!strcmp(furi_string_get_cstr(temp_str), "FuriHalSubGhzPresetCustom")) {
      if (!flipper_format_write_string_cstr(flipper_format,
                                            "Custom_preset_module", "CC1101")) {
        FURI_LOG_E(TAG, "Unable to add Custom_preset_module");
        res = SubGhzProtocolStatusErrorParserCustomPreset;
        break;
      }
      if (!flipper_format_write_hex(flipper_format, "Custom_preset_data",
    ...
    if (!flipper_format_write_hex(flipper_format, "Data", key_data,
                                  sizeof(uint64_t))) {
...
tpms_block_generic_deserialize(TPMSBlockGeneric *instance,
  ...
  do {
    ...
    uint8_t key_data[sizeof(uint64_t)] = {0};
    if (!flipper_format_read_hex(flipper_format, "Data", key_data,
                                 sizeof(uint64_t))) {
