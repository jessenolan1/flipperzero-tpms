...
void *tpms_protocol_decoder_schrader_gg4_alloc(SubGhzEnvironment *environment) {
  ...
  TPMSProtocolDecoderSchraderGG4 *instance =
      malloc(sizeof(TPMSProtocolDecoderSchraderGG4));
...
static bool
tpms_protocol_schrader_gg4_check_crc(TPMSProtocolDecoderSchraderGG4 *instance) {
  uint8_t msg[] = {
...
static ManchesterEvent level_and_duration_to_event(bool level,
                                                   uint32_t duration) {
  bool is_long = false;
  ...
  if (DURATION_DIFF(duration, tpms_protocol_schrader_gg4_const.te_long) <
    ...
    is_long = true;
  } else if (DURATION_DIFF(duration,
                           tpms_protocol_schrader_gg4_const.te_short) <
             tpms_protocol_schrader_gg4_const.te_delta) {
    is_long = false;
  } else {
...
void tpms_protocol_decoder_schrader_gg4_feed(void *context, bool level,
  ...
  case SchraderGG4DecoderStepReset:
    ...
    if ((level) &&
        (DURATION_DIFF(duration, tpms_protocol_schrader_gg4_const.te_long * 2) <
         tpms_protocol_schrader_gg4_const.te_delta)) {
...
SubGhzProtocolStatus tpms_protocol_decoder_schrader_gg4_serialize(
  ...
  furi_assert(context);
  TPMSProtocolDecoderSchraderGG4 *instance = context;
  return tpms_block_generic_serialize(&instance->generic, flipper_format,
                                      preset);
}
