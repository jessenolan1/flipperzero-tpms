...
void tpms_view_receiver_info_update(TPMSReceiverInfo *tpms_receiver_info,
                                    FlipperFormat *fff) {
  furi_assert(tpms_receiver_info);
  furi_assert(fff);

  with_view_model(
      tpms_receiver_info->view, TPMSReceiverInfoModel * model,
      {
...
void tpms_view_receiver_info_draw(Canvas *canvas,
  ...
  snprintf(buffer, sizeof(buffer), "%s %db",
  ...
  if (model->generic->battery_low != TPMS_NO_BATT) {
    snprintf(buffer, sizeof(buffer), "Batt: %s",
             (!model->generic->battery_low ? "ok" : "low"));
  ...
  if (furi_hal_rtc_get_locale_units() == FuriHalRtcLocaleUnitsMetric) {
    snprintf(buffer, sizeof(buffer), "%2.0f C",
             (double)model->generic->temperature);
    ...
    snprintf(buffer, sizeof(buffer), "%3.0f F",
             (double)locale_celsius_to_fahrenheit(model->generic->temperature));
    if ((model->generic->temperature < -27.77f) ||
        (model->generic->temperature > 37.77f)) {
      temp_x1 = 43;
      temp_x2 = 35;
    } else {
      temp_x1 = 41;
  ...
  snprintf(buffer, sizeof(buffer), "%2.1fbar",
           (double)model->generic->pressure);
  ...
  if ((int)model->generic->timestamp > 0 && model->curr_ts) {
    ...
    if (ts_diff > 60) {
      ...
      if (model->curr_ts % 2 == 0) {
        ...
        if (cnt_min >= 59) {
          canvas_draw_str_aligned(canvas, 106, 51, AlignLeft, AlignCenter,
                                  "Old");
        } else {
          snprintf(buffer, sizeof(buffer), "%dm", cnt_min);
          canvas_draw_str_aligned(canvas, 115, 51, AlignCenter, AlignCenter,
                                  buffer);
        }
      }
      ...
      canvas_draw_str_aligned(canvas, 112, 51, AlignCenter, AlignCenter,
                              buffer);
    }
  }
}
...
static void tpms_view_receiver_info_exit(void *context) {
  ...
  with_view_model(
      tpms_receiver_info->view, TPMSReceiverInfoModel * model,
      { furi_string_reset(model->protocol_name); }, false);
}
...
TPMSReceiverInfo *tpms_view_receiver_info_alloc() {
  ...
  view_allocate_model(tpms_receiver_info->view, ViewModelTypeLocking,
                      sizeof(TPMSReceiverInfoModel));
  view_set_context(tpms_receiver_info->view, tpms_receiver_info);
  view_set_draw_callback(tpms_receiver_info->view,
                         (ViewDrawCallback)tpms_view_receiver_info_draw);
  view_set_input_callback(tpms_receiver_info->view,
                          tpms_view_receiver_info_input);
  view_set_enter_callback(tpms_receiver_info->view,
                          tpms_view_receiver_info_enter);
  view_set_exit_callback(tpms_receiver_info->view,
                         tpms_view_receiver_info_exit);
  ...
  tpms_receiver_info->timer = furi_timer_alloc(
      tpms_view_receiver_info_timer, FuriTimerTypePeriodic, tpms_receiver_info);
