...
uint8_t tpms_scene_receiver_config_next_frequency(const uint32_t value,
                                                  void *context) {
  furi_assert(context);
  TPMSApp *app = context;
  uint8_t index = 0;
  for (uint8_t i = 0; i < subghz_setting_get_frequency_count(app->setting);
       i++) {
...
uint8_t tpms_scene_receiver_config_next_preset(const char *preset_name,
                                               void *context) {
  furi_assert(context);
  TPMSApp *app = context;
  uint8_t index = 0;
...
uint8_t tpms_scene_receiver_config_hopper_value_index(const uint32_t value,
...
static void tpms_scene_receiver_config_set_frequency(VariableItem *item) {
  ...
  if (app->txrx->hopper_state == TPMSHopperStateOFF) {
    ...
    app->txrx->preset->frequency =
        subghz_setting_get_frequency(app->setting, index);
  } else {
...
static void tpms_scene_receiver_config_set_preset(VariableItem *item) {
  ...
  tpms_preset_init(app, subghz_setting_get_preset_name(app->setting, index),
                   app->txrx->preset->frequency,
...
static void tpms_scene_receiver_config_var_list_enter_callback(void *context,
                                                               uint32_t index) {
  furi_assert(context);
  TPMSApp *app = context;
...
void tpms_scene_receiver_config_on_enter(void *context) {
  ...
  item = variable_item_list_add(
      ...
      "Frequency:", subghz_setting_get_frequency_count(app->setting),
      tpms_scene_receiver_config_set_frequency, app);
  value_index = tpms_scene_receiver_config_next_frequency(
      app->txrx->preset->frequency, app);
  scene_manager_set_scene_state(app->scene_manager, TPMSSceneReceiverConfig,
                                (uint32_t)item);
  ...
  item = variable_item_list_add(
      app->variable_item_list, "Hopping:", HOPPING_COUNT,
      tpms_scene_receiver_config_set_hopping_running, app);
  ...
  item = variable_item_list_add(app->variable_item_list, "Modulation:",
                                subghz_setting_get_preset_count(app->setting),
                                tpms_scene_receiver_config_set_preset, app);
  value_index = tpms_scene_receiver_config_next_preset(
      furi_string_get_cstr(app->txrx->preset->name), app);
  variable_item_set_current_value_index(item, value_index);
  variable_item_set_current_value_text(
      item, subghz_setting_get_preset_name(app->setting, value_index));

  variable_item_list_add(app->variable_item_list, "Lock Keyboard", 1, NULL,
                         NULL);
  variable_item_list_set_enter_callback(
      app->variable_item_list,
      tpms_scene_receiver_config_var_list_enter_callback, app);

  view_dispatcher_switch_to_view(app->view_dispatcher,
                                 TPMSViewVariableItemList);
}

bool tpms_scene_receiver_config_on_event(void *context,
