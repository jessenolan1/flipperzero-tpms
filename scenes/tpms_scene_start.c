...
void tpms_scene_start_on_enter(void *context) {
  ...
  submenu_add_item(submenu, "Read TPMS", SubmenuIndexTPMSReceiver,
                   tpms_scene_start_submenu_callback, app);
  submenu_add_item(submenu, "Relearn", SubmenuIndexTPMSRelearn,
                   tpms_scene_start_submenu_callback, app);
  // Help
  submenu_add_item(submenu, "About", SubmenuIndexTPMSAbout,
                   tpms_scene_start_submenu_callback, app);
...
bool tpms_scene_start_on_event(void *context, SceneManagerEvent event) {
  ...
  if (event.type == SceneManagerEventTypeBack) {
    ...
    scene_manager_set_scene_state(app->scene_manager, TPMSSceneStart,
                                  event.event);
  }
